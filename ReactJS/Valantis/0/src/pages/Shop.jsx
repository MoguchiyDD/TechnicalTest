...
function Shop() {
  ...
  const handleClick = async () => {
    ...
    const replaceSpaces = (string) => { return string.split(" ").join("%20") }
    const filter = (strFilter, query, condition, filter) => {
      if (condition === true) {
        const _filter = replaceSpaces(filter)
        if (query !== null) {
          curUrl = curUrl.replace(`${strFilter}=${query}`, `${strFilter}=${_filter}`)
        } else {
          curUrl += `&${strFilter}=${filter}`
        }
      } else {
        curUrl = curUrl.split(`&${strFilter}=${query}`).join("")
      }
    }
    ...
    if ((product !== null) && (price !== null) && (brand !== null)) {
      ...
      filter("product", queryProduct, isProduct.length >= 1, isProduct)

      const isPrice = price.value
      filter("price", queryPrice, isPrice > _prices[0], isPrice)
      
      const isBrand = document.querySelector("input[name=\"shop-brand\"]:checked").value
      filter("brand", queryBrand, isBrand !== import.meta.env.VITE_API_SHOP_BRAND_DEFAULT, isBrand)
